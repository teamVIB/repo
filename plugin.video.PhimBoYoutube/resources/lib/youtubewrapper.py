...
def return_youtubevideos(name,url,token,page,api_key,keyword):
	items_per_page = int(selfAddon.getSetting('items_per_page'))
	list_of_tupple_items = []
	items_per_page=50 #maxResult - fix -DO NOT change
	if page != 1:
		url_api = "https://raw.githubusercontent.com/tranduc33/youtubeapi-data/master/phim/"+keyword+"/sub/"+url+"PAGE"+str(page)+".txt"
		#import web_pdb; web_pdb.set_trace()
		#url_api = 'https://www.googleapis.com/youtube/v3/playlistItems?part=id,snippet,contentDetails&maxResults='+str(items_per_page)+'&playlistId='+url+'&key='+api_key +'&pageToken='+token
	else:
		url_api = "https://raw.githubusercontent.com/tranduc33/youtubeapi-data/master/phim/"+keyword+"/sub/"+url+".txt"
		#url_api = 'https://www.googleapis.com/youtube/v3/playlistItems?part=id,snippet,contentDetails&maxResults='+str(items_per_page)+'&playlistId='+url+'&key='+api_key
	#import web_pdb; web_pdb.set_trace()
